...
// path
std::string const path = "./histone_1/";

int main() {
  // std::random_device rd;
  // std::mt19937 gen(rd());
  std::mt19937 gen(0);
  std::uniform_int_distribution<int> random_index(2, n-1);
  ...
  for (int e : tq::trange(epochs)) {
    ...
    int const index_base_change = random_index(gen);
    for (int i = 2; i != n; ++i) {
      if (i != index_base_change) {
        ...
        auto theta = dna[i].theta() + oscillation(gen) * static_cast<double>(step);
        auto phi = dna[i].phi() + oscillation(gen) * static_cast<double>(step);
        auto psi = dna[i].psi() + oscillation(gen) * static_cast<double>(step);
