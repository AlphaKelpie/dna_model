...
int main(int argc, char* argv[]) {
  ...
  int n = 100;
  // path
  std::string path = "./rod_2/";
  // equilibrium distace between bases and rod (nm)
  double sigma_rod = 2.1;

  // Read parameters from file
  std::ifstream file("data_rod_2_" + std::string(argv[1]) + ".txt");
  if (file.is_open()) {
    file >> n;
    file >> epochs;
    file >> path;
  ...
  Output params = calculate_rod_2(dna, sigma_rod, n);
  ...
  for (int e : tq::trange(epochs)) {
    ...
    Output const params_new = calculate_rod_2(dna_new, sigma_rod, n);
