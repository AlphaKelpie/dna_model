...
\title{DNA coars-grain model to reproduce characteristics\\of higher-order structures\thanks{Chiral selection in wrapping, crossover, and braiding of DNA mediated by asymmetric bend-writhe elasticity\cite{main}}}
\author{Tommaso Rondini\thanks{Master's degree student in Physics, University of Bologna (Italy). Email: \url{tommaso.rondini@studio.unibo.it}}}
...
The study has explored elastic mechanisms for the selection of chirality in wrapping, crossover, and braiding of DNA based on a coarse-grained model.
...
I have recreated the code to reproduce their simulations.
...
Crossover and braiding are also important motifs of DNA for genome packaging, gene regulation, and enzyme recognition~\cite{br_1}.
...
It is clear that left-handed crossover implies right-handed braiding and vice-versa.
...
From Fig.~\ref{fig:braF_a} (middle) -when there is not soothing force- the braiding number mostly takes negative values, indicating that the left-handed braiding is preferred in this simulation.
Introducing the soothing force -let's see Fig.~\ref{fig:braF_b} and~\ref{fig:braF_c}- we see that the absolute values of $B$ tend to decrease as the tension increases.
...
When $F=$\SI{0}{\pico\newton} (Fig.~\ref{fig:braF_a}) the probability is distributed over a wide range, with some peaks in both negative and positive ranges but still higher probability for negative values.
With $F=$\SI{60}{\pico\newton} (Fig.~\ref{fig:braF_b}) the probability is narrower than that of Fig.~\ref{fig:braF_a}.
It indicates, as said above, that the soothing force reduce the braiding, but it is still biased towards negative values.
...
\section{Conclusions}
I have followed the studied paper that had investigated the elastic mechanisms of the selection of chirality in wrapping, crossover, and braiding of DNA based on a coarse-grained model and I have tried to reproduce their results.

The DNA model has consisted of two elastic chains that mutually intertwine in a right-handed manner forming a double-stranded helix with the distinction between major and minor grooves.
The model has been inclined to writhe in the left direction, even if the potential has no have asymmetric term: this was a consequence of the right-handed helix.
The left-handed wrapping has been statistically preferred around spherical core particle and uniform rod.
Monte Carlo simulations have suggested that two juxtaposed DNA molecules can braid each other spontaneously under moderate attractive interactions with the preference for left-handed braiding.

In general my simulations have reproduced their results, but in some cases there have been some discrepancies.
The peaks that I have founded in probability distributions of wrapping number $W$ and braiding number $B$ are not the same founded in the paper.
This suggest that the potential landscape has not been so regular and there have been plenty of attractor conformations that the model could take on.
...
It has used limited degrees of freedom, e.i. bending, dihedral and twist angles, but in reality may other internal degrees of freedom could also couple each other giving rise to more complex behaviours.
In addition, the constant used to determine bonding and bending energy (Eq.~\ref{eq:bond} and~\ref{eq:bend}), $K_{bond}$ and $K_{bend}$ have been determined to approximate as better as they could the bending persistence length  of real DNA, but they could be evaluated better.
Almost all the energy potential have been Morse potential: in reality the Coulomb potential had been the correct one, but it would have been to computational expensive.
