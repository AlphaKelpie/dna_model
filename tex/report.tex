...
\caption{$\sigma_{rod}=2.1$nm}
\label{fig:r1_a}
\end{subfigure}
\begin{subfigure}{.3\textwidth}
\includegraphics[width=\textwidth]{r1_B_2000000_17.pdf}
\includegraphics[width=\textwidth]{r1_B_wr_pr.pdf}
\caption{$\sigma_{rod}=3.1$nm}
\label{fig:r1_b}
\end{subfigure}
\begin{subfigure}{.3\textwidth}
\includegraphics[width=\textwidth]{r1_C_2000000_7.pdf}
\includegraphics[width=\textwidth]{r1_C_wr_pr.pdf}
\caption{$\sigma_{rod}=4.1$nm}
\label{fig:r1_c}
\end{subfigure}
\caption{Probability distribution of the wrapping number obtained from $20$ Monte Carlo simulations of $2\times 10^6$ steps (behind) and an example of the DNA wrapping model with $W_{rod}\simeq 0$ with $\sigma_{rod}=2.1$nm (a), \SI{3.1}{\nm} (b), and \SI{4.1}{\nm} (c). The rod local potential was used (Eq.~\ref{eq:rod_1}).}
\label{fig:r1}
\end{figure}
...
\end{equation}
where $\textbf{r}_{i}$ represent the position of the $i$-th node. They have introduced $N=100$ nodal points on the central axis of the rod (so in the z-axis), $\textbf{r}_{j}^{rod} \left (j=1,\dots,N\right )$, with equal separation of $b=$\SI{.34}{\nm}.
In Eq.~\ref{eq:rod_2} each node in the DNA model interacts with each node of the rod and vice versa.
They set $D_{rod}=5$pNnm and $\beta_{rod}=$\SI{2.0}{\per\nm}.
...
\label{fig:r2_c}
\end{subfigure}
\caption{Probability distribution of the wrapping number obtained from $20$ Monte Carlo simulations of $2\times 10^6$ steps (behind) and an example of the DNA wrapping model with $\sigma_{rod}=2.1$nm (a), \SI{3.1}{\nm} (b), and \SI{4.1}{\nm} (c). The rod global potential was used (Eq.~\ref{eq:rod_2}).}
\label{fig:r1}
\end{figure}
...
\begin{equation}\label{eq:bainding}
B=\dfrac{1}{2\pi}\sum_{i=1}^{N-1}sgn\left [\left (\textbf{C}_{i+1}-\textbf{C}_{i}\right )\cdot\left (\textbf{B}_{i}\times\textbf{B}_{i+1}\right )\right ]\sin^{-1}\left |\textbf{B}_{i}\times\textbf{B}_{i+1}\right |
\end{equation}
where
\begin{equation}
...
Fig.~\ref{fig:bra} shows examples of conformations (top), braiding number (Eq.~\ref{eq:bainding}) (middle), and probability distribution of the braiding number (bottom) in $20$ Monte Carlo simulation of $2\times 10^6$ steps, discarding the first $2\times 10^5$.
$D_{DNA}$ is set to \SI{0.7}{\pico\neuton\nano\meter}.
The $F$ parameter is set to \SI{0}{\pico\neuton} in Fig.\ref{fig:bra_a}, \SI{60}{\pico\neuton} in Fig.\ref{fig:bra_b}, and \SI{100}{\pico\neuton} in Fig.\ref{fig:bra_c}.
